...
# context, th_reward = get_now_reward(app_id, cache_list, cache_config)
def get_now_reward(cache_m):
    context = [1.0 for _ in range(18)]
    # cache_reward = [float(x) for x in cache_reward]
    th_reward = sum(float(x) for x in cache_m)/len(cache_m)
    # th_reward = 1.0 / th_reward
    return context, th_reward
# def get_now_reward(old_cache_reward, new_cache_reward):
#     # context_eg = [
#     #     np.array([[1.01699426, 0.18583102, -0.03044815, 0.41693462, 1.4612631,
#     #             -1.53857271, 0.67866136, 0.47406454, 1.31870985, 2.93602127,
#     #             0.91129102, 0.41019782, 0.98564628, 1.53317965, 2.40752894,
#     #             1.00679684, 1.11645485, 1.41644497]]),
#     #     np.array([[-1.76167626, -2.26518824, 0.49266518, -0.37237515, 0.40209199,
#     #             -2.20882347, 1.11166105, 0.77326353, 0.70173932, 7.25231613,
#     #             7.47418289, 0.7593369, 1.60706237, 4.39366529, 6.33183103,
#     #             4.49522716, 1.27610629, 0.85177314]]),
#     #     np.array([[0.18426885, 0.71057358, 0.92535247, 0.37148873, 1.09168757,
#     #             3.47055851, 0.12813143, 1.05207601, -1.55138936, 0.55536643,
#     #             4.71399529, 1.09002229, 1.09389608, 2.40762505, 0.15325196,
#     #             5.04801621, 3.59284635, 2.40667141]])
#     # ]

#     # value = 1.0  # 指定的特定值

#     # context = np.full_like(context_eg, value)
#     context = [1.0 for _ in range(18)]
#     if len(old_cache_reward) != len(new_cache_reward):
#         raise ValueError("Sequence lengths must be the same.")

#     old_cache_reward = [float(x) for x in old_cache_reward]
#     new_cache_reward = [float(y) for y in new_cache_reward]
#     th_reward = sum([(x - y) / x for x, y in zip(old_cache_reward, new_cache_reward)])
#     # cache_config_list = list(map(int, cache_config))
#     # cache_reward_list = list(map(float, cache_reward))
#     # product_list = [x * y for x, y in zip(cache_config_list, cache_reward_list)]  # 对应元素相乘
#     # print("product_list")
#     # print(product_list)
#     # th_reward = sum(product_list) / len(product_list)  # 平均值
#     return context, th_reward
...
# test success
def gen_init_cache_config(app_id, cache_arm_orders, llc_arm_orders, mb_arm_orders, NUM_UNITS,f_d_c):
    app_num = len(app_id)
    nof_cache = NUM_UNITS[0]
    nof_llc = NUM_UNITS[1]
    nof_mb = NUM_UNITS[2]
    cache_arms = 0
    llc_arms = 0 
    mb_arms = 0
    # 初始化core决策
    each_cache_config = nof_cache // app_num
    res_cache_config = nof_cache % app_num
    cache_config = [each_cache_config] * (app_num-1)
    if res_cache_config >= each_cache_config:
        for i in range(res_cache_config):
            cache_config[i]+=1
        cache_config.append(each_cache_config)
    else:
        cache_config.append(each_cache_config+res_cache_config)
    # core_config = [1, 1, 1, 1, 3]
    for i in range(len(cache_arm_orders)):
        if cache_arm_orders[i] == cache_config:
            cache_arms = i
            break
    ...
    # chosen_arms = [core_arms, llc_arms, mb_arms]
    chosen_arms = [cache_arms]
    f_d_c.writerow(cache_config)
    # f_d_c.writerow(llc_config)
    # f_d_c.writerow(mb_config)
    f_d_c.writerow(['*', '*', '*', '*', '*', '*', '*'])
    # 将config改成具体指令
    cache_list = refer_llc(cache_config)
    # llc_list = refer_llc(llc_config)
    ...
    return cache_list, chosen_arms, cache_config
...
def gen_nonOverlap_pool_config(NUM_APPS, NUM_UNITS):
    core_config = gen_configs_recursively(0, 0, 0, NUM_APPS, NUM_UNITS)
    for i in range(len(core_config)):
        other_source = np.array(core_config[i]).sum()
        core_config[i].append(NUM_UNITS[0] - other_source)